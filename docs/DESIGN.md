...
- `ssh/`: `SshControlTransport` — sshj `SSHClient` → exec `tmux -C new-session -t <session>` (+ optional `-L socket`/`-S path`); implements `ControlTransport`. Host key TOFU via `OpenSSHKnownHosts` in app files dir. `AndroidCrypto.install()` swaps the BC provider once at app start. Runs its own keepalive (`keepalive@openssh.com` every 15 s, dead after 4 unanswered) instead of sshj's thread, because it has to be stoppable: Android cuts a backgrounded app off the network while its socket stays open, and a keepalive that keeps counting misses there kills a healthy connection within a minute. `stopKeepAlive`/`startKeepAlive(probeFirst)` are driven by the session controller; a keepalive verdict is recorded in `failureReason` before the transport closes itself.
...
- `session/`: `SessionController` — owns SSH + `TmuxControlClient`, one `TerminalSession`/emulator per pane, bootstraps each pane with `capturePane`, feeds `%output` bytes to emulators, forwards typed bytes to `sendKeys`, parses `@salchang_meta` JSON per pane → window meta. Watches `ProcessLifecycleOwner` (via `SessionViewModel`): in the background it stops the transport's keepalive; on return it restarts it with an immediate probe, and a connection the transport reports as lost (`ConnectionState.Disconnected(lost = true)`, as opposed to tmux's `%exit` or the user's Disconnect) is re-attached automatically — right away if the app is visible and the link had been up ≥ 10 s, otherwise on the next return to the foreground. A re-attach selects the window that was current before, since a session joining a group starts on the group's lowest-index window.
